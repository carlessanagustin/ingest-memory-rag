...
# Values accepted by Haystack's DocumentSplitter.split_by.
...
def _env_split_by(name: str, default: SplitBy) -> SplitBy:
    raw = os.environ.get(name, "").strip()
    if not raw:
        return default
    allowed = get_args(SplitBy)
    if raw not in allowed:
        raise ValueError(f"{name}={raw!r} is invalid; expected one of {allowed}")
    return cast(SplitBy, raw)
...
class Settings:
    ...
    def from_env(cls) -> Settings:
        return cls(
            ...
            patterns=PATTERNS,
            split_by=_env_split_by("SPLIT_BY", DEFAULT_SPLIT_BY),
